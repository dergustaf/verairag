...
try:
    ...
    if st.button("Zeptej se kouče") and query:
        with st.spinner("🔍 Prohledávám archivy a připravuji odpověď..."):
            ...
            # 3. Vyhledávání v Pinecone napříč všemi jmennými prostory
            all_matches = []
            ...
            for m in all_matches:
                ...
                # Dynamické určení zdroje
                source = meta.get('title') or meta.get('session_id') or meta.get('file_name') or "Archivní záznam"
                context_parts.append(f"ZDROJ [{source}]: {text}")
                sources_info.append(f"- **{source}** (Míra shody: {m['score']*100:.1f}%)")

            # 4. Finální syntéza v češtině
